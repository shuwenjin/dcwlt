package com.dcits.dcwlt.pay.api.mq.event.exception;
...
import com.dcits.dcwlt.pay.api.mq.event.task.RspCodeMapTask;
...
public class EcnyTransException extends TransException {
    ...
    public static RspCodeMapDO convertRspCode(String srcId, String destId, String trxType, String errorCode, String errorMsg) {
        // 返回码实体
        RspCodeMapDO codeMap = RspCodeMapTask.getErrorCodeMap(AppConstant.ECNY_SYS_ID, srcId, destId, trxType, errorCode, "*");


        // 通用匹配
        if (codeMap == null && !trxType.equals(AppConstant.TXN_TYPE_COMM)) {
            codeMap = RspCodeMapTask.getErrorCodeMap(AppConstant.ECNY_SYS_ID, srcId, destId, AppConstant.TXN_TYPE_COMM, errorCode, "*");
        }

        // 交易类型兜底错误映射
        if (codeMap == null) {
            codeMap = RspCodeMapTask.getErrorCodeMap(AppConstant.ECNY_SYS_ID, srcId, destId, trxType, "*", "*");
        }


        //匹配兜底错误码
        if (codeMap == null) {
            codeMap = RspCodeMapTask.getErrorCodeMap(AppConstant.ECNY_SYS_ID, AppConstant.ECNY_SYS_ID, destId, AppConstant.TXN_TYPE_COMM, EcnyTransError.OTHER_TECH_ERROR.getErrorCode(), "*");
